...
def genQuarters(main_passage):
    quarter_cells = []
    prev_note = -3
    pre_prev_note = -2
    for main_cell in main_passage.cells:
...
def genAlbertiEighths(main_passage, leading_eighths = True):
    prev_note = 0
    main_cells = main_passage.cells
    cells = []
    for k in range(0, len(main_passage.cells)): #loop through all cells
